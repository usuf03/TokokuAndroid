...
class AdapterKeranjang(var activity: Activity, var data:ArrayList<Produk>, var listener: Listener):RecyclerView.Adapter<AdapterKeranjang.Holder>() {
    ...
    override fun onBindViewHolder(holder: Holder, position: Int) {
        ...
        var jumlah = data[position].jumlah // selalu buat variabel jika ingin selalu dipanggil beberapa kali
    ...
    private fun delete(data: Produk) {
        val myDb = MyDatabase.getInstance(activity)
        CompositeDisposable().add(Observable.fromCallable { myDb!!.daoKeranjang().delete(data) } // pilih yg io.reactivx
                .subscribeOn(Schedulers.computation())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe {
                })
